...
//&НаКлиенте
//Процедура ДлительнаяОперация(Период, ДоговорСсылка)
//ДлительнаяОперация = ВыполнитьДлительнуюОперацию(Период, ДоговорСсылка);
//	
//	Оповещение = Новый ОписаниеОповещения("ФункцияВыполнена", ЭтотОбъект);
//	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Оповещение);
//КонецПроцедуры
...
Функция ВыполнитьДлительнуюОперацию()
	...
	ПараметрыЗапуска.Вставить("Период", ЭтотОбъект.Период);

	ДлительнаяОперация = ДлительныеОперации.ВыполнитьПроцедуру(УникальныйИдентификатор, "ВКМ_РеализацияАгентскихДоговоровВызовСервера.ВКМ_СозданиеАбонентскихОтгрузок(Период)");
ДокументОбъект = РеквизитФормыВЗначение("Объект");
//ДлительнаяОперация = ДлительныеОперации.ВыполнитьПроцедуру(УникальныйИдентификатор, "ДокументОбъект.МассовоеСозданиеАбонентскихОтгрузок");
...
//Процедура СоздатьАктыНаСервере() Экспорт
...
//  
//  ВыполнитьДлительнуюОперацию(Объект.Период, ДоговорСсылка)
