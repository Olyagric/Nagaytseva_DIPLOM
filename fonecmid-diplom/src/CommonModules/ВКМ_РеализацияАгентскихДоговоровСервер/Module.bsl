...
Функция ВКМ_МассовоеСозданиеАбонентскихОтгрузок(ДатаНачала, ДатаОкончания) Экспорт
...
Выборка = РезультатЗапроса.Выбрать();
  ...
  ДанныеДляОбработки.Колонки.Добавить("Договор");
  ДанныеДляОбработки.Колонки.Добавить("Реализация");
  
 // Проверить повторное создание реализаций..
 
 
  Пока Выборка.Следующий() Цикл
Если Выборка.Реализация = "Пусто" Тогда
	НовыйДокумент = ВКМ_СоздатьРеализацию(Выборка.Договор, ДатаОкончания);
КонецЕсли;
 
    НоваяСтрока = ДанныеДляОбработки.Добавить();
	НоваяСтрока.Договор = Выборка.Договор;
	НоваяСтрока.Реализация = ?(Выборка.Реализация = "Пусто", НовыйДокумент, Выборка.Реализация);
...
Функция ВКМ_СоздатьРеализацию(Договор, ДатаОкончания) Экспорт
	
 НовыйДокумент = Документы.РеализацияТоваровУслуг.СоздатьДокумент();
 НовыйДокумент.Договор = Договор;
 НовыйДокумент.Дата = ?(ДатаОкончания<> Неопределено, ДатаОкончания,ТекущаяДатаСеанса());
...
//  Процедура ВКМ_МассовоеСозданиеАбонентскихОтгрузок(ДатаНачала, ДатаОкончания, АдресРезультата) Экспорт
//	
// Запрос = Новый Запрос;
//Запрос.Текст =
//	"ВЫБРАТЬ
...
//  Пока Выборка.Следующий() Цикл
//Если Выборка.Реализация = "Пусто" Тогда
//	НовыйДокумент = ВКМ_СоздатьРеализацию(Выборка.Договор);
//КонецЕсли;
// 
//    НоваяСтрока = ДанныеДляОбработки.Добавить();
//	НоваяСтрока.Договор = Выборка.Договор;
